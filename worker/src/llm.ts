...
- To create or intentionally replace a whole Doc body: call \`update_page_content\` with a \`markdown\` string. Headings, bullet/numbered/checklist items, and paragraphs are supported, as are inline **bold**, *italic*, \`code\`, and [links](url).
- To edit text already on a page: call \`get_page_components\` to find the blocks and their current content, then call \`update_block_content\` for one block or \`edit_page_content\` for several. Do not use \`update_page_content\` for a targeted edit: it replaces the whole body and changes every block identity.
