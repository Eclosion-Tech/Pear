...
async function main() {
  ...
  try {
    ...
    const result = await fetch(url, { method:'PUT', headers:{Authorization:`Bearer ${owner.token}`}, body:wasm });
    ...
    console.log('PASS: MCP question → durable request → authorized answer → addressed agent reply; duplicate, forged, closed-conversation and expired approval responses rejected.');
  } finally {
